...
# from schemas.response import error
...
#         return error(code=status.HTTP_400_BAD_REQUEST, message=e.message)
#
...
#         return error(code=status.HTTP_400_BAD_REQUEST, message=text)
#
...
#         text: str = '请求参数格式错误'
#         logger.error(f'{text}\nURL:{request.method}-{request.url}\nHeaders:{request.headers}\nerror:{e.errors()}')
#         return error(code=status.HTTP_422_UNPROCESSABLE_ENTITY, message=text)
#
#     @app.exception_handler(ValidationError)
#     async def inner_validation_exception_handler(request: Request, e: ValidationError):
#         text: str = '内部参数验证错误'
#         logger.error(f'{text}\nURL:{request.method}-{request.url}\nHeaders:{request.headers}\nerror:{e.errors()}')
#         return error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message=text)
#
